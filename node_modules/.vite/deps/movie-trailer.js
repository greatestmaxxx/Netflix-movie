import { n as __esm, r as __export, t as __commonJS } from "./chunk-D-sv5Gdm.js";

//#region node_modules/whatwg-fetch/fetch.js
var fetch_exports = /* @__PURE__ */ __export({
...
var init_fetch = __esm({ "node_modules/whatwg-fetch/fetch.js": (() => {
...
//#endregion
//#region node_modules/isomorphic-fetch/fetch-npm-browserify.js
var require_fetch_npm_browserify = /* @__PURE__ */ __commonJS({ "node_modules/isomorphic-fetch/fetch-npm-browserify.js": ((exports, module) => {
	init_fetch();
	module.exports = self.fetch.bind(self);
}) });

//#endregion
//#region node_modules/movie-trailer/index.js
var require_movie_trailer = /* @__PURE__ */ __commonJS({ "node_modules/movie-trailer/index.js": ((exports, module) => {
	(function(root, cx) {
		if (typeof define === "function" && define.amd) define(["isomorphic-fetch"], cx);
		else if (typeof exports === "object") module.exports = cx(require_fetch_npm_browserify());
		else root.movieTrailer = cx(root.fetch);
	})(exports, (fetch$1) => {
		...
		function getMovieId(search, options) {
			...
			return fetch$1("https://api.themoviedb.org" + encodeURI(path + options.apiKey + "&query=" + search + (options.year === null ? "" : "&year=" + options.year) + (options.language === null ? "" : "&language=" + options.language)), { method: "GET" }).then(handleFetchErrors).then((response) => response.json()).then((json) => {
				if (typeof json.status_message !== "undefined") throw new TypeError(json.status_message);
		...
		function getTrailer(movieId, options) {
			...
			return fetch$1("https://api.themoviedb.org" + encodeURI(path + movieId + "/videos?api_key=" + options.apiKey + (options.language === null ? "" : "&language=" + options.language)), { method: "GET" }).then(handleFetchErrors).then((response) => response.json()).then((json) => {
				if (typeof json.status_message !== "undefined") throw new TypeError(`movie-trailer: ${json.status_message}`);
				if (json.results.length === 0) throw new Error("No trailers found for that TMDB ID");
				let { results } = json;
				results = results.map((result) => {
					return result.key;
				});
				if (!options.id) results = results.map((videoId) => toUrl(videoId));
		...
		async function movieTrailer(movie, options, cb, legacy) {
			...
			if (!options) options = {};
			if (typeof movie !== "string" && !options.tmdbId) throw new Error("Expected first parameter to be a movie or TMDB ID (string)");
			else if (typeof options === "function") {
				cb = options;
				options = null;
			} else if (typeof options === "boolean" || options === "true") config.multi = options;
			...
			} else if (typeof options === "object") config = Object.assign(config, options);
			...
			if (cb) return result.then((response) => cb(null, response)).catch((error) => cb(error, null));
			return result;
		}
		return movieTrailer;
	});
